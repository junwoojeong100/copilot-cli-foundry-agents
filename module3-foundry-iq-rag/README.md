...
```python
# MCPTool은 azure-ai-projects에서만 제공됩니다 (azure-ai-agents에는 포함되지 않음).
...
> 💡 이 실습은 `azure.ai.agents.AgentsClient`(SDK v2)를 사용해 에이전트를 만들고, `tools=[mcp_tool]`로 `azure.ai.projects.models.MCPTool` 인스턴스를 그대로 전달합니다. `MCPTool`은 직렬화 가능한 Tool 모델이라 두 SDK가 호환됩니다.
...
모듈 4에서는:
- **Microsoft Agent Framework(MAF)**로 멀티 에이전트 시스템 구현 (Semantic Kernel + AutoGen 통합 프레임워크)
- 에이전트 간 **핸드오프(Handoff)** 패턴
- **GroupChat** / **커스텀 순차 워크플로우**
- 실제 업무 시나리오에 적용하는 방법
